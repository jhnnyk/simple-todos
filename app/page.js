      ...
      <div className='max-w-3xl mx-auto mt-10 p-8'>
        <div className='bg-white m-6 p-3 sticky top-0'>
          ...
          <div className='flex items-center gap-2 mt-10'>
            <input type="text" className='font-semibold placeholder:text-gray-500 
            border-[2px] border-black h-[60px] grow shadow-sm rounded-md 
            px-4 focus-visible:outline-yellow-400 text-lg transition-all 
            duration-300' placeholder='Add new todo' autoFocus />
            <button className='w-[60px] h-[60px] rounded-md bg-amber-500 flex justify-center items-center cursor-pointer transition-all duration-300 hover:bg-black/[0.8]'><AiOutlinePlus size={30} /></button>
          </div>
        </div>

        <div className='m-6 px-6'>
          {arr.map((todo, index) => (
            <div className='flex items-center justify-between mt-4' key={`todo-${index}`}>
              <div className='flex items-center gap-3'>
              <input id={`todo-${index}`} type='checkbox' className='w-4 h-4 accent-green-400 rounded-lg' />
              <label className='font-medium' htmlFor={`todo-${index}`}>Your todo list</label>
              </div>
              <div className='flex items-center gap-3'>
